import { apiUserURL } from '../../src/urls'
...
describe('Request', () => {
  it('Gets user data from the backend', () => {
    cy.request('POST', `${apiUserURL}/api/users`, userId).should(
      response => {
...
describe('Request', () => {
  it('Gets user data from the backend', () => {
    cy.request('PUT', `${apiUserURL}/google-oauth2|111661795839592168840`, role).should(
      response => {
        expect(response.status).to.eq(200)
        expect(response).to.have.property('headers')
        expect(response).to.have.property('duration')
